...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('tro-upms-systemid', systemid);
		$.cookie('tro-upms-systemname', systemname);
		$.cookie('tro-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('tro-upms-systemid') || 1;
	var systemname = $.cookie('tro-upms-systemname') || 'tro-upms-server';
	var systemtitle = $.cookie('tro-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
